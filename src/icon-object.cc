...
napi_value export_Icon_loadBuiltin(napi_env env, napi_callback_info info)
{
    ...
    NAPI_RETURN_NULL_IF_NOT_OK(napi_get_required_args(env, info, &id, &width, &height));
...
napi_value export_Icon_loadFile(napi_env env, napi_callback_info info)
{
    std::wstring path;
    int32_t width, height;
    NAPI_RETURN_NULL_IF_NOT_OK(napi_get_required_args(env, info, &path, &width, &height));
    ...
    napi_property_attributes attributes = napi_enumerable)
{
    ...
    {
        void* data;
        NAPI_THROW_RETURN_NULL_IF_NOT_OK(env, napi_get_cb_info(env, info, nullptr, nullptr, nullptr, &data));
    ...
    return napi_getter_property(utf8name, getter, attributes, (void*) (size_t) metric);
}
    ...
    napi_property_attributes attributes = napi_enumerable)
{
    auto getter = [](napi_env env, napi_callback_info info) -> napi_value
    {
        ...
        NAPI_THROW_RETURN_NULL_IF_NOT_OK(env, napi_get_cb_info(env, info, nullptr, nullptr, &this_value, nullptr));

        if (auto [status, wrapped] = IconObject::try_unwrap(env, this_value); status != napi_ok)
        {
        ...
        {
            napi_throw_type_error(env, nullptr, "'this' must be an instance of Icon.");
            return nullptr;
        }
        else
        {
            napi_value result;
            NAPI_THROW_RETURN_NULL_IF_NOT_OK(env, napi_create(env, wrapped->*member, &result));
            return result;
        }
    };
...
napi_status IconObject::define_class(EnvData* env_data, napi_value* constructor_value)
{
    return NapiWrapped::define_class(
        env_data->env,
        "Icon",
        constructor_value,
        &env_data->icon_constructor,
        {
            ...
            napi_method_property("loadBuiltin", export_Icon_loadBuiltin, napi_static),
    ...
    napi_value* result)
{
    ...
    if (auto [status, wrapper, wrapped] = NapiWrapped::new_instance(env, env_data->icon_constructor);
        status != napi_ok)
    {
        if (!shared)
        {

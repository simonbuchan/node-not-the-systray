...
bool apply_notification(notification_options* options, NOTIFYICONDATAW* nid,
                        IconData* icon_data) {
  nid->uFlags |= NIF_INFO;
...
bool apply_options(notify_icon_options* options, NOTIFYICONDATAW* nid,
                   IconData* icon_data) {
  if (options->hidden) {
...
napi_value export_add(napi_env env, napi_callback_info info) {
  ...
  if (id < 1 || id > 0xfffe) {
    napi_throw_range_error(env, nullptr,
                           "id out of range, must be between 1 and 0xFFFE.");
  }

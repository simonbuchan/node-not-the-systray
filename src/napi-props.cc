    ...
    std::initializer_list<napi_output_property> properties)
{
    ...
    for (const auto& prop : properties)
    {
        ...
        descriptors.push_back(napi_value_property(prop.utf8name, value, prop.attributes));
    }
    ...
    NAPI_RETURN_IF_NOT_OK(napi_define_properties(env, local, std::initializer_list(first, last) ));

...
std::wstring format_win32_message(HRESULT code) {
  ...
  FormatMessageW(FORMAT_MESSAGE_ALLOCATE_BUFFER |
                 ...
                 nullptr, code, LANG_USER_DEFAULT, (LPWSTR)&message_local, 0,
                 nullptr);
  if (!message_local) return L"Unknown error."s;

  // Trim ending \r\n
  auto len = wcslen(message_local);
  while (len && iswspace(message_local[len - 1])) --len;
  std::wstring result(message_local, len);
  LocalFree(message_local);
  return result;
}

napi_status napi_create_win32_message(napi_env env, HRESULT code,
                                      napi_value* result) {
  return napi_create(env, format_win32_message(code), result);
}

napi_status napi_create_win32_error(napi_env env, const char* syscall,
                                    HRESULT code, napi_value* result) {

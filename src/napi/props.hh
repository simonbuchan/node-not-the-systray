...
inline napi_status napi_define_properties(
  ...
  return napi_define_properties(env, object, properties.size(),
                                properties.begin());
}
...
inline napi_property_descriptor napi_getter_property(
    const char *utf8name, napi_callback getter,
    napi_property_attributes attributes = napi_enumerable,
    void *data = nullptr) {
  napi_property_descriptor desc = {};
  desc.utf8name = utf8name;
...
template <typename T>
inline napi_status napi_set_named_property(napi_env env, napi_value object,
                                           const char *utf8name,
                                           T &&native_value) {
  napi_value value = nullptr;
...
struct napi_erased_output {
  ...
  napi_status (*erased_get_value)(napi_env env, const void *native_data,
                                  napi_value *result);
...
struct napi_output_property {
  ...
  template <typename T>
  napi_output_property(const char *utf8name, const T &value,
                       napi_property_attributes attributes = napi_enumerable)
      : utf8name{utf8name},
